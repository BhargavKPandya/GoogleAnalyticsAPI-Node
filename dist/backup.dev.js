...
console.log(jwt);
jwt.authorize(function (err, tokerns) {
  if (err) {
    console.log(err);
  } else {
    console.log(tokerns);
...
process.env.GOOGLE_APPLICATION_CREDENTIALS = './google-auth.json'; //let test1 = getCustomDimensionList(account_id, web_property_id);
//console.log('Test1---');
...
function getAllKeys(jsonObject) {
  ...
  if (whatIsIt(spreadSheetData) === 'Array') {
    ...
    for (var s in temp) {
      ...
      var nestedJsonObject = jsonObject[s]; //console.log("nestedJsonObject" + JSON.stringify(nestedJsonObject));
  ...
  function extractFromJsonObject(offsetObject, nestedJsonObject) {
    var keys = Object.keys(nestedJsonObject); // console.log('Keys--' + keys);
...
for (var i = 0; i < demographics_info_JSON.length; i++) {
  ...
  jwt.authorize(function (err, tokerns) {
    ...
    var alreadyCD = getCustomDimensionList(account_ID, webPropertyID, allOffset).then(function (res) {
      //existingResult = res;
      console.log(res);
...
function getCustomDimensionList(accountID, webPropertyID, allOffset) {
  var offsets, accountId, webPropertyId, output, res;
    ...
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          ...
          _context.next = 5;
          return regeneratorRuntime.awrap(google.analytics('v3').management.customDimensions.list({
            //auth: jwt,
            "accountId": accountId,
            "webPropertyId": webPropertyId
          }));
